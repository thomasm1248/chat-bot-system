...
t.module('coreBrainLoader', () => {
  const e = {};

  e.getCoreBrainCodeAsync = async () => {
    return t.freeze([
        {
            source: 'test-code.chat',
            code: testBrainCode,
        },
    ]);
  };
  e.getCoreBrainCodeAsync.meta = `Returns\
 a list of strings, where each string is a\
 chat brain code file. They are returned in\
 the order they need to be processed.`;
